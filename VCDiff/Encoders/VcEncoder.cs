...
namespace VCDiff.Encoders
{
    ...
    public class VcEncoder : IDisposable
    {
        ...
        public VCDiffResult Encode(System.Threading.CancellationToken cToken, bool interleaved = false, ChecksumFormat checksumFormat = ChecksumFormat.None, IProgress<float>? progress = null)
        {
        ...
        /// <see cref="VCDiffResult.SUCCESS"/> if successful, <see cref="VCDiffResult.ERROR"/> if the sourceStream or target are zero-length.</returns>
        /// <exception cref="ArgumentException">If interleaved is true, and <see cref="ChecksumFormat.Xdelta3"/> is chosen.</exception>
		public async Task<VCDiffResult> EncodeAsync(System.Threading.CancellationToken cToken, bool interleaved = false, ChecksumFormat checksumFormat = ChecksumFormat.None, IProgress<float>? progress = null)
        {
            ValidateParameters(interleaved, checksumFormat);
            ...
            try
            {
                while (targetData.CanRead)
                {
                    ...
                    chunker.EncodeChunk(ntarget, outputStream, cToken);
                    progress?.Report((float)targetData.Position / targetData.Length);
